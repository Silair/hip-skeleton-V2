...
独立于稳态 anchor，仅在 **Tracking/Ramp** 检测起步趋势（`stop_probability < 0.4`、motion_confidence 上升、同向速度连续 N 帧、spread 增加），估计 `startup_frequency_prior_hz`，在 **Tracking→Ramp** 或 **Tracking/Ramp→Active** 时用 `startup_prior_gain`（默认 0.20）混合 `omega_target`（仍受 `max_omega_rate_rad_s2` 限速）。

CSV：`StartupPriorValid/Candidate/Applied/FrequencyHz/Confidence`。指标：`startup_prior_candidate_count`、`startup_prior_applied_count`。
